    ...
    def compute_phase_function_batched(
        ...
        if self.simulation.numerics.gpu:
            ...
            while not done:
                ...
                cuda.synchronize()
                e_field_theta_real[start_idx:start_idx+split_idx,:] = np.array(split_data[4].copy_to_host())
                e_field_theta_imag[start_idx:start_idx+split_idx,:] = np.array(split_data[5].copy_to_host())
                e_field_phi_real[start_idx:start_idx+split_idx,:] = np.array(split_data[6].copy_to_host())
